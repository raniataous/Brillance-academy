...
class Thim_Ekit_Widget_Course_Item_Progress extends Widget_Base {
						...
						foreach ( $settings['item_progress_repeater'] as $item ) {
							switch ( $item['item_progress'] ) {
								...
								case 'number':
									...
									break;
								case 'expire_time':
									$this->render_expire_time( $expiration_time );
									break;
								case 'progress_bar':
	...
	protected function render_expire_time( $expiration_time ){
		if ( ! $expiration_time ) {
			return;
		}
		?>
		<span class="expire-time">
			...
			<span class="time"><?php echo wp_kses_post( $expiration_time->format( get_option( 'date_format' ) ) ); ?></span>
		</span>
		<?php
	}
}
